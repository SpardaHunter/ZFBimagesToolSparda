...
class ZFBimagesTool(tk.Frame):
    ...
    def check_image_size(self, img_size=None):
        ...
        if img_size is None:
            img_size = (int(self.imgwidth_var.get() or 0), int(self.imgheight_var.get() or 0))
        else:
    ...
    def execute_create_zfb(self):
        if self.img_mode.get() == "manual":
            try:
                ...
                if width < 144 or height < 208:
                    messagebox.showerror("Image Size Error", "Image size must be at least 144 x 208.")
                    return
            except ValueError:
